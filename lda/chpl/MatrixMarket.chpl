...
class MMReader {
   ...
   proc read_header() {
      ...
      fmt1 = (header == "%%MatrixMarket matrix coordinate real general\n");
      fmt2 = (header == "%%MatrixMarket matrix array real general\n");
      assert((fmt1 == true || fmt2 == true), "attempted to load an unsupported file");
   ...
   proc read_matrix_info() {
      ...
      if fmt1 {
         ...
         var done = fin.readf("%i %i", nrows, ncols, nnz);
         nnz = if nrows == 1 then ncols else nrows;
         assert(done == true, "error reading matrix market file's information");
      }
